...
public partial class AssignedTrainings : ComponentBase
{
    ...
    {
        SetPageTitle();
        await GetAllAssignedTrainingCountForClient();
    }
    ...
    #region Assigned Client Counts
    private AssignedTrainingCountDto AssignedTrainingCountDto { get; set; } = new();
    
    private async Task GetAllAssignedTrainingCountForClient()
    {
        try
        {
            var response = await TrainingService.GetAllAssignedTrainingCountsForClient();
