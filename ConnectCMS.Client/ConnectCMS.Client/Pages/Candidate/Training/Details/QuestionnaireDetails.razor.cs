...
public partial class QuestionnaireDetails : ComponentBase
{
    ...
    private async Task GetAllAssignedTrainingInspectionsForCandidate()
    {
        try
        {
            var result = await TrainingInspectionService.GetAllAssignedTrainingInspectionsForCandidate(TrainingId, PageNumber, PageSize, Search);
    ...
    private async Task OpenTrainingInspectionQuestionnaireDetailsModal(Guid trainingInspectionId)
    {
        try
        {
            var result = await TrainingInspectionService.GetCandidateTrainingInspectionDetails(trainingInspectionId);
    ...
    private void NavigateToQuestionnaireAnswerUploadForm(Guid questionnaireId, string strategicType)
    {
        var strategyType = strategicType.ToInspectionTypeString();

        NavigationManager.NavigateTo(strategyType switch
        {
            InspectionType.SwotAnalysis => $"strategic-trait-questionnaire/{questionnaireId}",
            InspectionType.PersonalityTest => $"personality-test/{questionnaireId}",
            _ => $"candidate-answer-upload-form/{questionnaireId}"
        });
    }
